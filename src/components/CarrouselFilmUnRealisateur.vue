...
const props = defineProps<{
  id_realisateur: string
  max?: number
}>()
const { data: UnFilmRealisateur, error: ErrorUnFilmRealisateur } = await supabase
  .from('FilmRealise')
  .select(`id_film,FILM(Titre,url_images)`)
  .eq('id_realisateur', props.id_realisateur)
console.log(UnFilmRealisateur)
if (ErrorUnFilmRealisateur) {
  console.error(ErrorUnFilmRealisateur)
}
</script>
<template>
